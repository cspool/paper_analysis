...
**六个候选方向，两个已结案（D1、D6），首推 D3，支线 D2/D4，二期 D5。** 首推方向的技术路线成立，但**新颖性防线只剩三条**，且**所有 kernel 论证在真卡常量表建立之前都不能动笔**（§3）。

| # | 方向 | 状态 | 一句话理由 |
|---|---|---|---|
| **3** | **DiT block 级可训练三态门控 {重算｜rank-r 增量｜复用}** | **★★★ 首推** | 唯一 G1/C1/C2/C3 四段齐全且有可证伪目标的方向；**但已有直接先例（DiTPA ISCA'26），新颖性须收窄到三条** |
...
### 4.1 新颖性防线（必须在 intro 逐条挑明）

**已有直接先例：DiTPA（ISCA'26，开源，28nm / 4.37 mm² / 1.05 W）的控制结构就是逐去噪步的三态判定 {完整推理 | 跳过整轮 | 复用上轮输出}**，叠列稀疏、跑在可重构 PE 阵列上（S1 1.74× → +S2 2.90× → +S3 32.60×，均相对其自身 ASIC 基线；PE 利用率 98.36% vs GPU <20%）。差异是它做**动作规划 DiT，不是文生视频**，且是 ASIC 不是 GPU。

**→ D3 的新颖性只剩三条**：
1. **中间态是 rank-r 增量**，而非特征复用；
2. **门控可训练**，而非固定阈值；
3. 在 **GPU 文生视频**同设置下超过现有栈。
...
**本地对照组全缺**：Chipmunk / TaylorSeer / ToCa / DisCa / LearniBridge / TeaCache / AdaCache **全 vault 零命中** —— 必须自行补抓并复现。
...
| **(b) 相似度随 schedule 长度的衰减曲线** | **D3 的 6.72× 基线要分 50 步 / 4–8 步两档报，这条曲线是分档依据**；vault 完全没有 |
...
- [ ] P1：补抓本地零命中的对照组 —— Chipmunk / TaylorSeer / ToCa / DisCa / LearniBridge / TeaCache / AdaCache
