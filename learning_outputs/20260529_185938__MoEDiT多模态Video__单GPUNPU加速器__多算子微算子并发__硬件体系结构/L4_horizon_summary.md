...
| **软件流水线** | **TileLang T.Pipelined** | 编译器自动分析 buffer 依赖→插入架构特定异步指令 (Ampere→cp.async, Hopper→TMA, CDNA→buffer_load_dword) + 自动 warp specialization | 70 行 Python FA = 98% FA-3 性能；跨三架构统一 pipeline annotation | Q4.2 |
