...
| MoE调度 | Faster MoE Expert Skipping/Pruning | Expert skipping (na 6→2): throughput up to 1.32×；Expert pruning (ne 64→16): up to 2.3× speedup；SGLang v0.4.4 post1 on A800/H200 with DeepSeek-V2-Lite/V3 | 动态减少激活expert数/剪枝冗余expert | Q2.6, vault: Faster MoE (2480.89) |
...
| KV-Cache | Streaming KV Cache Dual-Level (LiveStar) | Intra-dialogue (clip内) + Inter-dialogue (跨clip) 双级KV Cache；Peak-End压缩（保留高PPL峰值+最新帧）；SVeD Swap：响应→沉默时swap cache末尾位置 | 5min video FPS 3.82 (Both) vs 2.50 (No Cache)，1.53×加速 | Q2.1, vault: Streaming KV Cache (556.9) |
