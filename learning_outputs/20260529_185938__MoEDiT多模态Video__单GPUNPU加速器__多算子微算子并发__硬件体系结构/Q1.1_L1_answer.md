...
for each MoE layer l = 1..L:                         # L=32 for Mixtral
    ...
    <!-- ! 类似QK计算token之间的相关性，QR计算token和e之间的关联。 -->
    x_norm = RMSNorm(x)                              # [T, d_model]
    ...
    <!-- ! 将 T 个 token 按 routing 结果重排为 E 组，将路由到e的token拼接成张量后，计算FFN -->
    for e = 0..E-1:
    ...
    <!-- ! 按token计算，从out_e中拆出属于特定token的行，结合TopK分数加权组合。 -->
    y = zeros([T, d_model])
    for t = 0..T-1:
        for k = 0..K-1:
            e = topk_indices[t, k]
...
with torch.no_grad():
    ...
    <!-- ! 每个patch[p,p,3]是1个区域,image[H,W,3]变成[H/p*W/p,3*p*p],embed作投影到patches[H/p*W/p,3*p*p] -->
...
for l = 1..L:                                        # L=32 for LLaVA-v1.5
    ...
    <!-- ! image-tokens@*是否用于image-tokens的稀疏计算?类似自蒸馏,但text为主的任务的计算收益不高 -->
