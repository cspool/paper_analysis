...
3. Model Runner — 逐层 Transformer 执行:
   ...
   for layer ℓ in 0..L-1:
       ...
       // KV_cache: [num_blocks, block_size, num_kv_heads, head_dim]
        ...
        <!-- ? MHA Decoder:token(hidden-dim)进入MHA(M个head)后,拆分后计算M个q(head-dim),M个k和M个v(head-dim) ,读取M组历史token的kv的KV-Cache(head-dim),计算M组(i=1...M)qi*(Ki,ki)和si*(Vi,vi),将M个ki和vi加入KV-Cache. -->
       ...
       for each query token q_i:
           ...
           for block_id in block_table[req_id]:
               ...
               // Online Safe Softmax
               <!-- !按block(序列中一部分k/v tokens)读取和计算完整序列的softmax -->
       ...
       <!-- ! Expert是FFN,和KV-Cache无关,对Mo而言,block就是token集合 -->
       if is_moe_layer:
...
<!-- ! 每个MoE layer有8个7B的Expert, TRT 是 “编译优化层”，把模型从通用框架里的算子图，变成能用 Tensor Core/量化/融合 kernel 的执行 engine.-->
===========================================================
...
Step 3: INT4 量化注入
  ...
  for each expert_weight in model.experts:
      ...
      <!-- ! 权重按int4存储,加速传输,计算时反量化到w16,和a16使用fp16*fp16的core计算 -->
...
for layer ℓ in 0..31:
    ...
    <!-- ! 假设B=1,只看1个token -->
    h = RMSNorm(h)                              # [1, 4096]
    <!-- ! fp16 input - fp32 accum - fp16 clip -->
    Q,K,V = FusedQKV(h, W_qkv_int4[ℓ])          # INT4 CUTLASS GEMM
    <!-- ! CUTLASS INT4 MMA: m16n8k32的tile shape, 解包 INT4→fp16 accum→FP16 epilogue -->
    attn_out = FlashAttention3(Q,K,V)           # [1, 4096]
    h = h + attn_out

    # MoE Block
    gate_logits = Router(h)                     # FP16 (保持 routing 精度)
    <!-- ! [B,4096] * [4096,N-E] = [B, N-E] -->
    topk_e, topk_w = TopK(softmax(gate_logits), K=2)
    <!-- ! [B, 2], topk-e是每个token所用的expert-id, topk-w是每个token使用expert输出的权重 -->

    output = 0
    for expert_id in topk_e:
        # Expert FFN: 3× INT4 GroupedGEMM
        <!-- ! int4->fp16, fp16*fp16的MMA -->
        gate_out = INT4_GEMM(h, W_gate[expert_id])          # [1, 14336]
        <!-- ! h[1, 4096] * w_gate[4096, 14336] = gate[1, 14336] -->
        up_out   = INT4_GEMM(h, W_up[expert_id])            # [1, 14336]
        <!-- ! h[1, 4096] * w_up[4096, 14336] = up[1, 14336] -->
        hidden   = SiLU(gate_out) * up_out
        <!-- ! SiLU(gate)是概率张量[1, 14436], 概率张量[1, 14436]和up[1, 14436]逐元素相乘得到hidden[1, 14436] -->
        down_out = INT4_GEMM(hidden, W_down[expert_id])     # [1, 4096]
        <!-- ! hidden[1, 14336] * w_down[14336, 4096] = down[1, 4096] -->
...
<!-- !
     !不同并行方式, 是不同维度的切分方式, 对一个初始计算过程, 不同并行方式将部分过程进行重写, 按照后端支持的并行方式和算法需求的运行资源调度对齐.
     
     !隔离实例之间不通信, 不同隔离实例的数据隔离, 并行方式是不同隔离实例作相互无关的计算并行.
     !比如DP是不同请求的推理并行, 隔离实例之间的重复存储, 降低可用的存储上限.

     !隔离实例内部作通信, 内部的通信实例的数据通过传输协作, 并行方式是不同通信实例作协作关系的计算并行.
     !比如PP是不同layer的pipeline并行, TP是大矩阵乘法layer中不同head/hidden/channel的并行, EP是多矩阵乘法layer中不同矩阵乘法的并行, CP是大存储需求layer中不同存储子块的并行. 通信实例之间的通信开销, 降低可能的性能上限.

     ?DP(data/batch): 将 batch 切分成 request 集合分给不同 DP 实例, 复制“完整逻辑模型” #DP 份。DP 实例之间一般不通信。

     ?TP(tensor): 把 Attention / Linear / FFN layer内的权重矩阵按 hidden/head/channel 维度切成 #TP 份。同一个 token/request 需要多个 TP rank 合作计算，需要 AllReduce / AllGather / ReduceScatter, 合并同一个算子的部分结果。

     ?EP(expert): 把 MoE layer 按 expert 维度切成 #EP 份。token 根据 router 被发送到对应 expert 所在 GPU，所以需要 All-to-All Dispatch 和 Combine, 发送batch内不同token到目标Expert, 并回收Expert结果。
     
     ?PP(pipeline): 将 Model 按 layer 维度切分, 不同 layer 的权重放置不同计算实例, 不同计算实例传递 layer 输出, 协作完成多个 layer 的计算. 
     
     ?CP(context): 将 Sequence(tokens) 拆分成不同段 subSequence, 每段放到不同实例计算, Sequence的KV-Cache存储分拆到不同实例, 每一个实例的subSequence计算需要其他实例的KV-Cache. -->
...
<!-- ! SGLang 主要由三层组成：
     ! API / 编程接口层：你可以像 OpenAI API 一样调用，也可以用 SGLang 的 Python/native interface 写结构化生成程序。
     ! Serving 调度层：continuous batching、chunked prefill、prefill/decode 调度、MoE 并行等。
     ! KV cache / kernel 层：RadixAttention 管理共享前缀 KV cache，底层可用 FlashInfer、Triton、FlashAttention 等 attention backend。 -->

<!-- ! 适合优先用 SGLang 的场景：
     ! 多个请求共享长 system prompt、few-shot examples、tool schema。
     ! Agent / tool calling / 多轮对话，需要反复把同一上下文前缀喂给模型。
     ! 需要 OpenAI-compatible local/private serving。
     ! 需要高吞吐在线服务，而不是单条离线推理。
     ! MoE 模型推理，尤其要结合 EP、DeepEP、FlashInfer 等系统优化。  -->
...
<!-- ! Triton	"我告诉你 block 做什么，你（编译器）自己决定 warp/thread 怎么干"
     ! TileLang	"我可以像 Triton 一样写 block 代码，也可以在需要时下探到 warp/thread 细节"
     ! CUTLASS	"每一层、每个 warp、每个 thread、每条指令，我全都要管——这才是极致性能"-->
...
<!-- ! grouped gemm将不同expert的不同token数量打包成1个kernel, 输入可以打包用满算力, 但是4个expert的权重需要全部加载到HBM. -->
...
<!-- ! 解析deepspeed(训练框架)的MoE大模型的模型元数据, 得到推理计算过程-->
2. GPU-efficient Token Routing:
   CUB radix sort (按 expert ID 排序 token)
   + CUTLASS GroupedGEMM (合并各 expert 的 GEMM)
<!-- ! cub是基础算法(sort)的并行加速库. cutlass是kernel模板库,模板提供硬件编程接口. cublas是黑盒高性能线性代数库. cudnn是黑盒高性能DNN算法库. -->
3. INT4/INT8 Fused GEMM+Dequantize kernel
4. MoE Decoder Batch Pruning
<!-- ! batch中结束生成EOS的请求被tokens sort剔除参与GroupedGEMM -->
5. Triton Inference Server 集成 (云规模弹性部署)
<!-- ! nvidia triton serving是为了连续批处理, 不等于 OAI triton language -->
```
...
<!-- ! 离线调很多固定shape的 micro-kernel template，在线拼 micro-kernel组成动态大shape的kernel  -->
