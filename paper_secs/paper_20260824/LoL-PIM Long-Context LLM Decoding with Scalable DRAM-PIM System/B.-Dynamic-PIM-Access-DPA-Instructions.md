...
This dynamic and compact representation provides an advantage over prior PIM systems ( [\[16\]](#page-18-1), [\[21\]](#page-18-4), [\[54\]](#page-20-6), [\[62\]](#page-20-10)), which must generate instruction sequences whose size grows linearly with the token length. As shown in Fig. [10\(](#page-7-0)c), this linear growth creates severe instruction buffer pressure and a scalability bottleneck for long-context inference. In comparison, DPA's encoding ensures the instruction size remains small and nearly constant regardless of context length, thereby avoiding command buffer bloat and enabling scalability to long context.
